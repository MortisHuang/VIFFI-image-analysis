...
for file in file_list_lym[:read_number]:
    im = Image.open(file)
    imarray = np.array(im)
    nplus_img_list.append(imarray)
lym_img_array = np.asarray(nplus_img_list)
lym_img_label = np.ones(lym_img_array.shape[0])*0

for file in file_list_neu[:read_number]:
    im = Image.open(file)
    imarray = np.array(im)
    nminus_img_list.append(imarray)
neu_img_array = np.asarray(nminus_img_list)
neu_img_label = np.ones(neu_img_array.shape[0])*1

#%% Combine two class

img_arr   = np.concatenate((neu_img_array, lym_img_array), axis = 0)
img_label = np.concatenate((neu_img_label, lym_img_label), axis = 0) 
...
#%%
model.save('{}Whitecell_model.h5'.format(result_path)) 
...
pandas.DataFrame(hist).to_excel(historysavepath  + 'Whitecell_VGG16_training_history.xlsx', index=False)
...
ax.legend()
plt.savefig("{}Fig4c_Whitecell_tSNE.png".format(result_path))
#%% Save tSNE Result
import pandas as pd

writer = pd.ExcelWriter('{}Fig4c_Whitecell_VGG16_tSNE_Result.xlsx'.format(result_path))
